...
export const Select = (props: Props) => {
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="flex items-center border border-border-light dark:border-border-dark rounded h-10 px-2 mt-3 mb-4">
                <select
                    name={name}
                    ...
                    {...register && register(name, { required: required ?? false })}>
                    {
                        options?.map((option, index) => (
                            <option
                                className="bg-bg-light-1 dark:bg-bg-dark-1"
                                key={index}
